import React from "react";
// import ShowSection from "./ShowSection";
import Showitem from "./Showitem";
import { connect } from "react-redux";
import { bindActionCreators } from "redux";
import { fetchData, removeAdd, addRemove } from "./actionCreators";
import "./Showlist.css";
/* eslint-disable */

class Showlist extends React.Component {
  constructor(props) {
    super(props);
    this.handleChange = this.handleChange.bind(this);
  }
  componentDidMount() {
    console.log("START componentDidMount");
    this.props.fetchData();
    // console.log(this.props);
    console.log("END componentDidMount");
  }
  handleChange(id) {
    let toggle = false;
    this.props.recommendations.forEach(val => {
      if (val.id === id) {
        toggle = true;
      }
    });
    if (!toggle) {
      this.props.removeAdd(id);
    } else {
      this.props.addRemove(id);
    }
  }

  render() {
    console.log("this.props_show list", this.props);
    let { isList, isShowInRec, countRunTime } = this.props;
    console.log("this.props", this.props);
    const arrProps = [];
    const arrCur = [];
    let arrInx = 0;
    let runTimes = 0;
    const tyleList = ["mylist", "recommendations"];
    for (let ele in this.props) {
      tyleList.forEach(str => {
        if (str === ele) {
          arrProps.push([str, this.props[ele]]);
        }
      });
    }
    console.log("arrProps", arrProps);
    const curlist = arrProps.map(arr => {
      if (arrInx !== 0) {
        isList = false;
        countRunTime++;
      }
      if (countRunTime % 2 !== 0) {
        isShowInRec = !isShowInRec;
      }

      arrInx++;
      console.log("countRunTime", countRunTime);
      console.log("curisList", isList);
      console.log("curisShowInRec", isShowInRec);
      // const curStyle = isList ? {} : { display: "none" };
      // console.log("curStyle", curStyle);
      return arr.map(list => {
        return (
          <li>
            <Showitem
              key={list.id}
              // img={list[0].img}
              // title={list[0].title}
              {...list}
              isList={isList}
              isShowInRec={isShowInRec}
              handleChange={this.handleChange}
              // style={curStyle}
            />
          </li>
        );
      });
    });
    return (
      <div className="main">
        <div>
          <h2>MyList:</h2>
          <div className="showList">{curlist}</div>
        </div>
      </div>
    );
  }
}
function mapStateToProps(stateRedux) {
  // debugger;
  return {
    mylist: stateRedux.mylist,
    recommendations: stateRedux.recommendations,
    isList: true,
    isShowInRec: false,
    countRunTime: 0
  };
}
function mapDispatchToProps(dispatch) {
  // console.log("dispatch", dispatch);
  return bindActionCreators({ fetchData, removeAdd, addRemove }, dispatch);
}
export default connect(mapStateToProps, mapDispatchToProps)(Showlist);
